...
public:
	...
	*/
	static void loadImage(SDL_Renderer*& renderer, std::vector<Texture*>& tabTexture, Uint8 stateScreen, Uint8 select,
		const std::string &path, const std::string &msg, Uint8 alpha, int x, int y, unsigned int w, unsigned int h, Uint8 cnt = nocenter);
...
public: // opérations sur l'objet et affichage
	...
	virtual bool renderTextureTestString(const std::string& msg, int xc = -1, int yc = -1);
	...
	virtual bool renderTextureTestStringAndStates(const std::string& msg, Uint8 stateScreen, int xc = -1, int yc = -1);
